/**
 * Security & Secret Shape Guard for Technocore Console V4
 * Protects private keys, seeds, and recovery phrases from accidental transmission
 * while permitting legitimate SHA-256 hashes, protocol state hashes, and signatures.
 */

// BIP-39 standard wordlist sample subset for fast detection of seed phrases
const COMMON_BIP39_WORDS = new Set([
  ...
  'squeeze', 'squirrel', 'stable', 'stadium', 'staff', 'stage', 'stair', 'stamp', 'stand', 'start',
  ...
  'utility', 'vacant', 'vacuum', 'vague', 'valuable', 'van', 'vanish', 'vapor', 'various', 'vast',
...
// Protocol field names known to contain legitimate 64-char SHA-256 hashes
...
/**
 * Check if a sequence of words constitutes a BIP-39 recovery seed phrase
 * @param {string[]} words
 * @returns {boolean}
 */
...
/**
 * Redesigned Secret Shape Guard:
 * Detects whether message text contains sensitive material such as private keys, seed phrases, or raw key secrets.
 * Allows legitimate 64-character SHA-256 hashes, Sonnet state hashes, and protocol JSON.
 *
 * @param {string} text
...
export function detectSensitiveContent(text) {
  ...
  // 1. PEM private key block header (CRITICAL: Private key detected)
  const pemRegex = /-----BEGIN (?:[A-Z0-9_-]+ )?PRIVATE KEY-----/i;
  if (pemRegex.test(trimmed)) {
    return {
      sensitive: true,
      reason: 'PEM private key block detected',
      description: 'This message contains a PEM private key header. Never send or transmit private keys.'
    };
  }

  // 2. Clear private key labels/prefixes with hex or base64 key
  // e.g. "secret: ...", "private_key: ...", "sk: ...", "nsec1...", "seed: 3f8a...", "key: ..."
  // Strictly exclude public "did:key:" identifiers
  const labeledSecretRegex = /(?<!did:)\b(?:secret_key|private_key|priv_key|privkey|seed|nsec1|key)\b\s*[:=]\s*["']?([0-9a-fA-F]{64,128}|[a-zA-Z0-9+/=_-]{43,88})/i;
  if (labeledSecretRegex.test(trimmed)) {
    return {
      sensitive: true,
      reason: 'Labeled private key or seed credential detected',
      description: 'The message contains text explicitly labeled as a private key or seed. Remove sensitive credentials before dispatching.'
    };
  }
  ...
  // 4. 64-byte Ed25519 Secret Key in Hex (128 consecutive hex characters)
  const hex128Regex = /\b[0-9a-fA-F]{128}\b/;
  if (hex128Regex.test(trimmed)) {
    return {
      sensitive: true,
      reason: '64-byte secret key detected (128 hex characters)',
      description: 'This message contains a 128-character hexadecimal string matching a full 64-byte Ed25519 secret key. Remove the secret key before sending.'
    };
  }

  // 5. Check 64-character hex strings:
  // Legitimate: SHA-256 hashes, previous_state_hash, poem_sha256, Merkle roots, dictionary/manifest hashes
  // Suspicious: Unlabeled standalone 64-char hex or seed pasted as credential
  const hex64Matches = trimmed.match(/[0-9a-fA-F]{64}/g);
  if (hex64Matches && hex64Matches.length > 0) {
    ...
    for (const match of hex64Matches) {
      // If it is inside a JSON protocol message under an allowed hash field name, it is 100% permitted
      if (isJson && jsonObj && typeof jsonObj === 'object') {
        let fieldAllowed = false;
        for (const [key, value] of Object.entries(jsonObj)) {
          if (typeof value === 'string' && value.includes(match)) {
            const lowerKey = key.toLowerCase();
            if (HASH_FIELD_NAMES.has(lowerKey) || lowerKey.includes('hash') || lowerKey.includes('sha256') || lowerKey.includes('digest')) {
              fieldAllowed = true;
              break;
            }
          }
        ...
        if (fieldAllowed) {
          continue; // Allowed protocol hash
        }
      }

      // Check context in text around the 64-char hex match
      const matchIndex = trimmed.indexOf(match);
      const startIdx = Math.max(0, matchIndex - 40);
      const endIdx = Math.min(trimmed.length, matchIndex + match.length + 40);
      const context = trimmed.slice(startIdx, endIdx).toLowerCase();

      // Explicit hash indicators in context => Allowed
      if (
        context.includes('hash') ||
        context.includes('sha256') ||
        context.includes('sha-256') ||
        context.includes('digest') ||
        context.includes('previous_state') ||
        context.includes('poem_sha') ||
        context.includes('manifest') ||
        context.includes('dictionary') ||
        context.includes('cmudict') ||
        context.includes('root') ||
        context.includes('merkle') ||
        context.includes('txid') ||
        context.includes('tclk') ||
      ...
      // Explicit secret indicator in context => BLOCKED
      if (
        context.includes('secret') ||
        context.includes('private') ||
        context.includes('seed') ||
        context.includes('priv') ||
        /\bkey\b/i.test(context)
      ) {
        return {
          sensitive: true,
          reason: '32-byte raw secret credential detected',
          description: 'This message contains a 64-character hex string indicated as a secret key or seed. Remove the raw credential before sending.'
        };
      }

      // Standalone or ambiguous 64-char hex string:
      // Per prompt: "If ambiguous, show a warning instead of silently blocking legitimate protocol traffic."
      return {
        sensitive: false,
        warning: true,
        reason: 'Unlabeled 64-character hex string detected',
        description: `This message contains a 64-character hex string (${match.slice(0, 8)}...${match.slice(-6)}). If this is a SHA-256 hash or protocol ID, you can proceed. Ensure you did not accidentally paste a private seed.`
      };
    }
  }

  // 6. 86-char or longer Base64url outside allowed protocol context
  if (trimmed.includes('secretKey') || trimmed.includes('privateKey')) {
    return {
      sensitive: true,
      reason: 'Private key variable detected',
      description: 'Message contains a reference or payload representing private key data.'
    };
  }
...
/**
 * Wipe byte array in-memory for security
 * @param {Uint8Array} array
 */
export function wipeBytes(array) {
  if (array && array.fill) {
    array.fill(0);
  }
}
...
/**
 * Redact sensitive credential strings from logs or errors
 * @param {string} str
 * @returns {string}
 */
